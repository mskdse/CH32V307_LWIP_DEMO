...
static void WCHNET_PhyPNProcess(void)
{
    ...
    if((ETH->MMCRGUFCR == 0) && (ETH->MMCRFCECR >= 3))
    {
        ...
        { uint32_t mto = 1000; while((ETH->MMCCR & ETH_MMCCR_CR) && (--mto)); }  // wait, with timeout
    }
    if(ETH->MMCRGUFCR != 0)
    {
        PhyPolarityDetect = 0;
        ...
        if(phy_bcr & PHY_AutoNegotiation)   //determine whether auto-negotiation is enable
        {
            if(phy_anlpar == 0)
            {
                ...
                if(phy_bsr & PHY_AutoNego_Complete)
                {
                    ...
                    /* no printf in ISR (stack) - netif task reports link */
                    EXTEN->EXTEN_CTR &= ~EXTEN_ETH_10M_EN;
                    phyLinkReset = 1;
                    phyLinkTime = LocalTime;
                    LinkSta = 0;
 ...
 *
 * @param   len : [out] frame length, excluding the 4-byte CRC
 *
 * @return  pointer to the frame, NULL when no valid frame is pending.
 */
 ...
 *
 * @return  pointer to the TX buffer, NULL if the DMA still owns it.
 */
 ...
 *
 * @brief   Restore normal MAC address filtering. ETH_LinkUpCfg enables
 *          promiscuous + receive-all for the raw (no-stack) demo; for
 *          lwIP the MAC filter should be active.
 */
...
static void WCHNET_ETHIsr(void)
{
    ...
    if( int_sta & ETH_DMA_IT_NIS )
    {
        ...
        {
            /* no printf here: this ISR runs with WCH software-stack saving
             * on the interrupted task's stack; keep it lean */
            ETH_PHYLink( );
...
void ETH_Init( uint8_t *macAddr )
{
     ...
     * tick never nest, and both use software stack saving (interrupt()).
     * Hardware stack saving (WCH-Interrupt-fast) only has 3 levels and
     * conflicts with FreeRTOS task scheduling, so it is not used here. */
